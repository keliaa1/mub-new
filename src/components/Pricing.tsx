    ...
    <section id="pricing" className="py-24 bg-[#0B0D0F]">
      <div className="max-w-7xl mx-auto px-6 md:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {/* Basic Plan */}
          <div className="border border-white/10 rounded-3xl p-10 hover:shadow-2xl transition-all bg-white/5 backdrop-blur-sm group">
            ...
            <ul className="space-y-4 mb-8">
              {['LLC Articles of Organization', 'Operating Agreement', 'EIN Registration Support', 'Email Support'].map((feature, i) => (
                <li key={i} className="flex items-center gap-3 text-gray-300">
                  <svg className="w-5 h-5 text-orange-500/60 group-hover:text-orange-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                  {feature}
                </li>
              ))}
            </ul>
          ...
          {/* Premium Plan */}
          <div className="border border-white/20 rounded-3xl p-10 bg-white text-[#0B0D0F] shadow-2xl relative overflow-hidden">
            ...
            <ul className="space-y-4 mb-8">
              {['Everything in Basic', 'Registered Agent Service', 'Annual Report Filings', 'Priority 24/7 Support'].map((feature, i) => (
                <li key={i} className="flex items-center gap-3 text-gray-800">
                  <svg className="w-5 h-5 text-orange-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                  {feature}
                </li>
              ))}
            </ul>
